...
- **Ignored:** every rendered artefact — `*.html`, `*_files/`, `*.parquet`, `*.csv`, `*.rds`,
  `*.gpkg`, and validation `*.pdf` maps under `data/validation/`.
...
monotone. See `doc/audits/geo_ids/santiago_geography_opus_5.md`.
...
- Artefacts for a city go under `data/validation/<city>/<data_type>/`.
