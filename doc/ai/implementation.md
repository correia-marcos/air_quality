# Structural migration: implementation and evidence

Implemented in the working tree based on revision `720f6412761bbfdcdaba008a8909bc2ce71b4889` on 7 September 2026. Changes are uncommitted and await human review. Analytical formulas, samples, weighting and missingness specifications were preserved. Existing audits retain their evidential value and original scope.
...
2. **Shared agent guidance.** `doc/ai/` is canonical. Root entry points and Claude commands/rules/reviewers route to it. Seven Codex skills, two reviewer definitions, minimal project configuration, command rules and hook wiring were added. Common guard policy lives in `scripts/ai/guard_policy.py`; client adapters retain supported native decisions.
...
- **Clean source-to-geography preparation remains a known boundary.** Existing geographic builders still combine cached extraction with acquisition. Fresh interim directories may expose this dependency. No automatic download or silent fallback to the old derived raw location was added. Complete offline preparation and its input-access documentation must be demonstrated before clean-room acceptance.
